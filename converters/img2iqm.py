...
def load_images(folder, show):
    limages = []
    lnames = []
    for filename in os.listdir(folder):
        if filename.startswith(".") is False:
            file = os.path.join(folder, filename)
            lnames.append(os.path.splitext(os.path.basename(file))[0])
            npimg = np.array(Image.open(file).convert('RGB'))  # you can pass multiple arguments in single line
            # print(npimg.shape)
            ...
            if npimg is not None:
                limages.append(npimg)
    return limages, lnames


inputDir = str(sys.argv[1])  # input with all images to convert to iqm
outputCsv = str(sys.argv[2])  # output file name

imgs, names = load_images(inputDir, False)
...
for i, img in enumerate(imgs):
    ...
    print(type(img))
    print(img.shape)
    iqm = compute_quality_features(img).astype(str)
    ...
    iqm = np.insert(iqm, 0, label, axis=0)
    iqm = np.insert(iqm, 1, names[i], axis=0)
    print(iqm)
    print(iqm.shape)

    # to add np.array row to dataframe
    iqm_row = pd.Series(iqm)
    iqm_row_df = pd.DataFrame([iqm_row])
    df = pd.concat([iqm_row_df, df], ignore_index=True)
